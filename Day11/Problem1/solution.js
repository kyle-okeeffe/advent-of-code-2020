 ...
 */
const getAdjSeatStats = (seat, seatArrangement) => {
  const stats = {
    '.': 0,
    L: 0,
    '#': 0
  ...
  [
    ...
    const state = getSeatState([seat[0] + offset[0], seat[1] + offset[1]], seatArrangement);
    if (state.length === 1) {
      stats[state]++;
    }
  });
...
const seatingChange = (seat, seatArrangement) => {
  ...
  const adjStats = getAdjSeatStats(seat, seatArrangement);
  if (seatState === 'L' && adjStats['#'] === 0) {
    return '#';
  } else if (seatState === '#' && adjStats['#'] >= 4) {
    return 'L';
  }
  return '';
};
